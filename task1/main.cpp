...
public:
    ...
    ParabolicSolver(string bound_type, int N, int K, double T)
...
int main() {
    ...
    for (string bound_type: {"appr1", "appr2", "appr3"}) {
        auto solver = ParabolicSolver(bound_type, N, K, T);

        map<string, vector<vector<double>>> results;

        results["implicit"] = solver.implicit(N, K, T);
        results["explicit"] = solver.eexplicit(N, K, T);
        results["niklson"] = solver.niklson(N, K, T);
        results["analytic"] = solver.analytic(N, K, T);
        matplot::figure();
