...
const double omega = 1.8; // Параметр верхней релаксации
const double epsilon = 1e-6; // Критерий останова метода простых итераций
...
int main() {
    ...
    while (error > epsilon) {
        ...
        for (int i = 1; i < Nx - 1; ++i) {
            for (int j = 1; j < Ny - 1; ++j) {
                double newU = (1 - omega) * u[i][j] + omega * 0.25 * (u[i - 1][j] + u[i + 1][j] + u[i][j - 1] + u[i][j + 1]
    ...
    std::ofstream outputFile("result_prost_iter.txt");
